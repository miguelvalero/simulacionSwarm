...
def send_telemetry_info():
    ...
    while sending_telemetry_info:
        ...
        lock.acquire()
        external_client.publish(sending_topic + "/telemetryInfo/"+port+"/", json.dumps(get_telemetry_info()))
        lock.release()
        time.sleep(0.25)
...
def process_message(message, client):
    ...
    if command == "connect":
        if state == 'disconnected':
            ...
            vehicle = connect(connection_string, wait_ready=False, baud=baud)
...
def AutopilotService (connection_mode, operation_mode, puerto, external_broker, username, password):
    global op_mode
    global external_client
    global internal_client
    global state
    ...
    internal_client = mqtt.Client("Autopilot_internal_"+port)
    internal_client.on_message = on_internal_message
    internal_client.connect("localhost", 1884)


    external_client = mqtt.Client("Autopilot_external_"+port, transport="websockets")
    ...
    if connection_mode== "global":
        if external_broker == "hivemq":
            ...
            external_client.connect("broker.emqx.io", 8083)
    ...
    print("Waiting....")
    external_client.subscribe("+/autopilotService/"+port+ "/#", 2)
...
if __name__ == '__main__':
    ...
    AutopilotService(connection_mode,operation_mode, puerto, external_broker, username, password)
